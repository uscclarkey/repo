...
def on_demand_list(url):
    ...
    for a in all_videos:
        ...
        if ' Series' in title:
            ...
            addDirPlayable(title,url,203,thumb,"",plot, "", "od")
    addDir("[COLOR gold]>> Next Page[/COLOR]",np_url,201,xbmc.translatePath(os.path.join('special://home/addons/plugin.video.streamboxlive', 'art', 'next.png')), '','')
    setView('episodes', 'episodes-view')
...
def disney_jr(url):
    ...
    for c in categories:
        ...
        if name == 'A POEM IS HOME IN THE FASHION':
            thumb = xbmc.translatePath(os.path.join('special://home/addons/plugin.video.streamboxlive', 'art', 'disney_junior.jpg'))
