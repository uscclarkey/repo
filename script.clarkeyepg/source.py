...
class Database(object):
        ...
        while True:
            ...
            try:
                ...
                if cancel_requested_callback is None:
                    xbmc.log('[script.clarkeyepg] Database is locked, bailing out...', xbmc.LOGDEBUG)
                    break
                else:  # ignore 'database is locked'
                    xbmc.log('[script.clarkeyepg] Database is locked, retrying...', xbmc.LOGDEBUG)

            except sqlite3.DatabaseError:
                self.conn = None
                if self.alreadyTriedUnlinking:
                    xbmc.log('[script.clarkeyepg] Database is broken and unlink() failed', xbmc.LOGDEBUG)
                    break
                else:
                    try:
    ...
    def _updateChannelAndProgramListCaches(self, date, progress_callback, clearExistingProgramList):
        ...
        c = self.conn.cursor()
        try:
            xbmc.log('[script.clarkeyepg] Updating caches...', xbmc.LOGDEBUG)
...
class XMLTVSource(Source):
    ...
    def __init__(self, addon):
        ...
        self.logoFolder = 'http://raw.github.com/uscclarkey/repo/master/logos/'
        self.xmltvFile = 'http://raw.github.com/uscclarkey/repo/master/guide.xmltv'
        self.xmltvUrl = 'http://raw.github.com/uscclarkey/repo/master/guide.xmltv'
        self.xmltvType = 0
